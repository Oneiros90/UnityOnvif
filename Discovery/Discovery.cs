using Onvif.Discovery.Client;
using Onvif.Discovery.Common;
using Onvif.Discovery.Exceptions;
...
#pragma warning restore 4014
		...
		DiscoveryDevice ProcessResponse (UdpReceiveResult response, Guid messageId)
		{
			if (response.Buffer != null) {
				...
				if (IsFromProbeMessage (messageId, xmlResponse)
					...
					return CreateDevice (xmlResponse.Body.ProbeMatches[0]);
				}
			}
			return null;
		}
		...
		DiscoveryDevice CreateDevice (ProbeMatch probeMatch)
		{
			Uri uri = new Uri(probeMatch.XAddrs);
			string scopes = probeMatch.Scopes;

			return new DiscoveryDevice
			{
				Model = Regex.Match(scopes, "(?<=hardware/).*?(?= )")?.Value,
				Mfr = ParseMfrFromScopes(scopes),
				XAdresses = ConvertToList(uri.AbsoluteUri),
				Address = uri.Host,
				Port = uri.Port,
